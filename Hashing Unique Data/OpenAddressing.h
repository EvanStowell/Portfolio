    ...
    void rehash(int &collisions) {
        ...
        for (int i = 0; i < oldTable.size(); ++i) {
            if (oldTable[i].status == FILLED) {
                insert(oldTable[i].key, oldTable[i].value, collisions);
            }
        }
...
public:
    ...
    void insert(string key, Keyable value, int &collisions) {
        if (!find(key, collisions)) {
            ...
            int count = 0; //counts times of read filled indices for quadratic probing
            while (table[index].status == FILLED) {
                // Add count^2 to the index for quadratic probing
                ++count;
                index += count*count - ((count-1)*(count-1));
                index %= table.size();
            }
            table[index].key = key;
    ...
    // Find
    optional<Keyable> find(string key, int &collisions) const {
        // Hash the key to get an index
        unsigned long index = hornerHash(key);
        int count = 0; //for quadratic probing
